...
upwindFD(const Eigen::VectorXd &u0,
    ...
    time = Eigen::VectorXd::LinSpaced(nsteps+1, 0, T);

    /* Main loop */
    double const k = 0.5 * dt / dx;
    for (auto t = 0; t < nsteps; ++t) {
        apply_boundary_conditions(u, t);
        for (auto i = 1; i < N+1; ++i) {
            double const a_val = a(xL + (i)*dx);
            double const a_abs = std::max(a_val, 0.0) - std::min(a_val, 0.0);
            u(i, t+1) = u(i, t) + k * (-a_val * (u(i+1, t) - u(i-1, t)) + a_abs * (u(i+1, t) - 2*u(i, t) + u(i-1, t)));
        }
    }
...
centeredFD(const Eigen::VectorXd &u0,
    ...
    /* Main loop */
    double const k = dt / (2 * dx);
    for (auto t = 0; t < nsteps; ++t) {
        apply_boundary_conditions(u, t);
        for (auto i = 1; i < N+1; ++i) {
            u(i, t+1) = - k * a(xL + (i-1)*dx) * (u(i+1, t) - u(i-1, t)) + u(i, t);
        }
    }
...
int main() {
    double T = 2.0;
    double dt = 0.01; // Change this for timestep comparison
    int N = 101;
